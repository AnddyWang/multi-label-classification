...
tf.app.flags.DEFINE_integer('batch_size',2,'the batch size during training the network')
...
def main(unused_argv):
    with tf.Graph().as_default():
        ...
        images, labels = tf.train.batch([image, label], batch_size=FLAGS.batch_size, num_threads=2, capacity=50000)
        ...
        with tf.Session() as sess:
            ...
            val_steps_per_epoch = int(math.ceil(50 / FLAGS.batch_size))
            ...
            prediction_text=prediction_arr[:,13]
            label_text=label_arr[:,13]
